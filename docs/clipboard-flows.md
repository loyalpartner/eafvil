...
**文件**：`crates/emskin/src/handlers/xwayland.rs` + `crates/emskin/src/clipboard_bridge.rs`
...
宿主变更先从第 1/2/3 节进 `clipboard_bridge::inject_host_selection`：
...
**文件**：`crates/emskin/src/clipboard_bridge.rs` + `handlers/xwayland.rs`
...
| `Host` | `ClipboardBackend::receive_from_host` | `ClipboardBackend::receive_from_host` |
...
- 内部 C-X `SetSelectionOwner` → `XwmHandler::new_selection`：`origin = X11`（同时喂 smithay dd + `ClipboardBackend::set_host_selection`）
...
外部 X owner   X server / XFixes    X11ClipboardProxy     inject_host_      smithay dd     X11Wm         内部 X client
                                    (emskin-clipboard::x11)    selection         (总线)         (代理窗口)
  │                 │                     │                   │                │             │                 │
...
外部 W app   host compositor   ClipboardProxy(独立连接)    inject_host_    smithay dd     X11Wm          内部 X client
                                (emskin-clipboard::data_control)            selection       (总线)         (代理窗口)
  │              │                    │                        │              │             │                   │
...
外部 W app   host compositor   WlDataDeviceProxy(共享 winit conn)  inject_host_  smithay dd   X11Wm      内部 X client
                                (emskin-clipboard::wl_data_device)                  selection     (总线)       (代理窗口)
  │              │                          │                           │            │          │                │
  │              │ (用户先把焦点给 emskin 主窗口)                         │            │          │                │
  │              │ keyboard.enter(serial=S) │                           │            │          │                │
  │              ├─────────────────────────►│ latest_serial = S         │            │          │                │
...
内部 X client  X11Wm        XwmHandler       smithay dd    X11ClipboardProxy    X server       外部 X client
                            ::new_selection  (总线)         (emskin-clipboard::x11)
    │           │                │                │                │                │                │
    │ SetSelectionOwner          │                │                │                │                │
    ├──────────►│                │                │                │                │                │
    │           │ new_selection  │                │                │                │                │
...
内部 X client  X11Wm   XwmHandler       smithay dd    ClipboardProxy(独立连接)  host compositor   外部 W app
                      ::new_selection   (总线)        (emskin-clipboard::data_control)
    │           │          │                │               │                        │                  │
    │ SetSelectionOwner    │                │               │                        │                  │
    ├──────────►│          │                │               │                        │                  │
    │           │new_sel.  │                │               │                        │                  │
...
client         ::new_sel.     (总线)       (emskin-clipboard::wl_data_device)
  │       │       │                │               │                              │                │
  │       │       │                │               │ keyboard.enter/key           │                │
  │       │       │                │               │ latest_serial = S            │                │
  │       │       │                │               │◄─────────────────────────────┤                │
