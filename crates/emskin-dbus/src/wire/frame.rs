...
mod tests {
    ...
    fn parse_round_trip_method_call() {
        ...
        assert_eq!(parsed.headers.path.as_deref(), Some("/org/freedesktop/DBus"));
        assert_eq!(
    ...
    fn parse_borrowed_body_zero_copy() {
        let request = hello_call();
        let owned = FrameBuilder::method_return(&request).serial(2).body(&42u32).build();
